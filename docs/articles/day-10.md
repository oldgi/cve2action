...
昨天留了一個沒解的矛盾：Juniper 那個漏洞，EPSS 說未來 30 天遭利用的機率只有 1.7%，但我說它在 KEV 裡。

今天把 KEV 抓下來，看清楚這兩個數字在講什麼。
...
這個差別不只是效率，它改變了「查不到」的意思。

NVD 查不到某個 CVE，我們只知道自己不知道。但**目錄在手上、裡面沒有這個 CVE，那是明確事實**。所以 KEV 狀態是三態，不是兩態：

- `LISTED`：在目錄裡。
- `NOT_LISTED`：目錄在手上，它不在裡面。
- `UNKNOWN`：**目錄根本沒抓到**——只有這種情況才是不知道。

多寫一個檢查：目錄自己宣告 `count: 1721`，我比對實際筆數。對不上就整批拒收。拿一份被截斷的目錄去判斷「這個漏洞不在清單裡」，比沒有目錄更危險。
...
兩個都對，因為它們回答不同問題。EPSS 預測**未來 30 天的廣度**，那是掃射器和 IDS 看得到的東西；KEV 記錄**過去發生過的事實**，哪怕只發生在少數幾個目標。針對性攻擊推不高 EPSS，但足以進 KEV。

**兩個不在清單裡。** OpenSSH 那個使用者列舉，還有 Rockwell PLC 的 `CVE-2024-6242`——後者 EPSS 是 0.095，percentile 0.95，已經贏過 95% 的 CVE，但 CISA 沒收它。
...
三個資料源接齊了：NVD 給嚴重度、EPSS 給預測、KEV 給事實。

但它們**都還沒進公式**。今天引擎算出來的排序，跟 Day 8 一模一樣。

這是刻意的。三個訊號怎麼加權、KEV 要不要直接拉高優先序、EPSS 和 KEV 衝突時聽誰的——那是 Day 14 的題目，而且需要規則寫在設定檔、留 ADR。先把資料接乾淨，再談怎麼用。

二十二個新測試：三態語意、目錄殘缺拒收、`Unknown` 不等於 `False`、逾時不寫檔。

## 明天

資料齊了，但我還沒有一間公司可以套。到目前為止的資產情境都是我隨手編的七行 CSV。
