...
一家虛構的線上服務商：DMZ 六台、應用四台、資料三台、管理三台，加上辦公端點、機房環控 PLC 與實驗室 wiki。四個 Crown Jewel：客戶與帳務資料庫、備份庫、網域控制站。八項控制措施，每項都要寫出**證據**——證明不了效果就是 `UNKNOWN`。
...
那是個以 CVSS 10.0 聞名的網域控制站漏洞。翻開快照才發現：它的兩筆 v3.1 **都標 Secondary**——微軟 5.5、NVD 自己 10.0——唯一的 `Primary` 是我不解析的 v2。Day 8 的規則「Primary 優先，其餘照順序」於是讓排前面的 5.5 勝出。
...
兩層原因。我挑 CVE 偏好有名的，三十個裡二十四個在 KEV，這不像真實環境；更根本的是公式——一半權重給了曝險與業務，而這家公司十三台 CRITICAL、六台對外，光這兩項就把底分推到五分以上。
