...
四十筆發現。**公司是假的，漏洞不是**：三十個 CVE 都有 NVD、EPSS、KEV 的真實快照，分數由腳本從快照讀進 CSV，我一個都沒手打；重跑不打網路，輸出逐位元組相同。

## 資料一放大，程式就露餡

七行 CSV 沒事，換成三十個 CVE 就壞了兩處。

**第一個是限流。** 抓到第十五個開始整批 429。Day 7 設的 6.5 秒間隔剛好卡在 NVD「30 秒 5 次」的滾動窗邊界，前十個僥倖過關，後面全被擋。改成 7.5 秒，429 時退避三十秒重試一次。
...
這正是 Day 8「還沒做到的」裡我自己寫下的缺口：兩個 Secondary 打架沒處理。當時七個 CVE，踩不到。改法是加一層 **NVD 自評優先**，並更新 ADR。

## 跑完四十筆

![result](../images/day-11-result.png)

三十八筆評分，兩筆 `NEEDS_CONTEXT`：一台不在清冊裡的影子 NAS，一筆 NVD 沒給 v3.1、掃描器也沒給值的 TLS 老問題。另有一筆掃描器說 9.9、NVD 說 9.8，`reason` 留著 `scanner said 9.9`。

控制的對照很乾淨：同樣的 `CVE-2021-41773`、同樣對外、同樣關鍵業務，兩台入口網站只差 WAF 有沒有證據——有的 **8.4 High**，沒有的 **9.9 Critical**。隔離實驗室的 9.8 掉到 **6.4 Medium**。

但整體分布是個問題：**八個 Critical、二十二個 High、八個 Medium，一個 Low 都沒有。**
...
四十筆發現只對應三十個 CVE。Log4Shell 一個就佔了四台機器，EternalBlue 三台——排序表上是七筆獨立的列，實際上可能只是兩件修補工作。

Day 1 講過「三筆 Finding 不等於三個風險」，今天總算有一份夠大的資料能看見它：引擎現在還是一筆一列地排，歸併成修補工作是 Day 25 的題目。
