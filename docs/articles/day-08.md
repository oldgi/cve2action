...
順帶一提，昨天留的原始回應今天派上用場：五份舊快照都不用重抓。
...
**版本偏好一換，Medium 變 High。** Juniper 路由器放在 INTERNET × PARTIAL × CRITICAL 的位置：偏好 3.1 時用 4.4，算出 6.45 **Medium**；把偏好改成 4.0，用 6.7，算出 7.6 **High**。
