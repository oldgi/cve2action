...
| 5 | 先決定 30 天做什麼，也決定不做什麼 | MVP 規格 | 完成範圍與 DoD |
| 6 | 打開 NVD：取得 CVE 與 CVSS 資料 | NVD Collector | 可快取、可重跑 |
| 7 | CVSS v3.1、v4.0 怎麼共存？ | 統一 CVSS Model | 保留 score 與 vector |
| 8 | EPSS：漏洞未來遭利用的可能性 | EPSS Collector | 處理查無資料與逾時 |
| 9 | CISA KEV：哪些漏洞已被實際利用？ | KEV Collector | 正確辨識 KEV |
| 10 | 建立一間不存在的數位公司 | 模擬資料 v1 | 無真實企業資訊 |
| 11 | Internet、DMZ、內網：暴露程度怎麼量化？ | Exposure Score | 四種區域可重現 |
| 12 | 核心系統和測試機不能一視同仁 | Asset Impact | 關鍵資產排序合理 |
| 13 | 第一版風險公式：先讓它可以被挑戰 | Risk Engine v0.1 | 相同輸入必得相同結果 |
| 14 | EPSS、KEV 與 CVSS 各自該占多少？ | Likelihood Score | 顯示每項貢獻值 |
| 15 | 如何衡量資料敏感度與營運衝擊？ | Impact Score | 業務因子有尺度定義 |
| 16 | WAF、EDR、網路隔離能減多少風險？ | Control Score | 控制不能把風險降為零 |
| 17 | 風險分數不能是黑箱 | Explain API | 可回傳公式與理由 |
| 18 | 五個漏洞的人工判斷與模型比較 | 校準測試 | 差異都有書面解釋 |
...
## 20. Day 2 設計修訂：從排序延伸到可交辦行動

修訂日：2026-09-16。依據：[ADR-0003](decisions/ADR-0003-條件式行動決策.md)及 [Day 2 案例備忘錄](research/day-02-case-design.md)。本節補充既有決策輸出與資料契約，不改寫 CRPS 權重，也不聲稱已實作風險引擎。

### 20.1 決策輸出

每筆決策除優先分數與原因外，須保留行動類型（修補、緩解、人工審查、事件處置）、負責團隊、行動期限、修補期限、相容性限制、控制證據與重評條件。這些行動可並行，不強迫所有案件只能做單一排序。

風險接受須有獨立的狀態、權責核准者及到期日。接受單號不代表已核准；未知或過期控制不能自動降低優先序。完成緩解、獲准暫緩或排定窗口，均不等於已完成修補。

### 20.2 證據與路徑

- 所有離線案例使用固定 scenario_as_of；資料採集時間、期限與模型時間分開。
- 已知遭利用、針對性證據與掃描活動分欄保存，不把它們當成互斥規模或直接映射為成功率。
- 未找到路徑須標示模型範圍，不能寫成不存在路徑；網路連線、身分授權及利用條件需區別。
- 共用跳板、掃描來源或其身分出現異常，須通知依賴這些控制的其他案件重新評估。
- 控制證據需包含來源／目的／埠／協定／方向、驗證時間與證據參照。Day 2 僅有模擬摘要，後續需補完整資料模型。
